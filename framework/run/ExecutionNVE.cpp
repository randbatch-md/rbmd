...
void ExecutionNVE::PostSolve() {}

void ExecutionNVE::InitialCondition()
{
...
void ExecutionNVE::ComputeForce()
{
  ComputeAllForce();
}

void ExecutionNVE::ComputeAllForce()
{
...
void ExecutionNVE::UpdateVelocity()
{
  try
  {
...
void ExecutionNVE::UpdatePosition()
{
...
void ExecutionNVE::SetCenterTargetPositions()
{
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::LJForce()
{
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::EleNearForce()
{
  if (_use_erf == true)
  {
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::NearForce()
{
  _nearforce_type = _para.GetParameter<std::string>(PARA_NEIGHBOR_TYPE);
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::NearForceLJ()
{
  _nearforce_type = _para.GetParameter<std::string>(PARA_NEIGHBOR_TYPE);
  if (_nearforce_type == "RBL")
  {
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::NearForceEAM()
{
  _nearforce_type = _para.GetParameter<std::string>(PARA_NEIGHBOR_TYPE);
  if (_nearforce_type == "RBL")
  {
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::BondForce()
{
  // original
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::AngleForce()
{
  //angle
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::DihedralsForce()
{
  //dihedrals
...
vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::SpecialCoulForce()
{
  ...
  {
    RunWorklet::ComputeSpecialCoul(
      box, _atoms_id, groupVecArray, _force_function, _topology, _locator, _spec_coul_force);
  }
  return _spec_coul_force;
}

vtkm::cont::ArrayHandle<Vec3f> ExecutionNVE::EleNewForce()
{
...
void ExecutionNVE::ComputeTempe()
{
...
void ExecutionNVE::SetForceFunction()
{
  InitERF();
  if (_para.GetParameter<bool>(PARA_FAR_FORCE))
  {
...
void ExecutionNVE::InitParameters()
{
...
void ExecutionNVE::TimeIntegration() {}

void ExecutionNVE::ConstraintA()
{
  auto angle_list = _para.GetFieldAsArrayHandle<Id>(field::angle_atom_id);
  auto&& anglelist_group = vtkm::cont::make_ArrayHandleGroupVec<3>(angle_list);
...
void ExecutionNVE::ReadPotentialFile(std::ifstream& input_file)
{
...
void ExecutionNVE::AllocateEAM() {}

void ExecutionNVE::file2array()
{
  Id i, j, k, m, n;
  Real sixth = 1.0 / 6.0;
...
void ExecutionNVE::interpolate(Id n, Real delta, std::vector<Real>& f, std::vector<Vec7f>& spline)
{
  for (int m = 1; m <= n; m++)
  {
    spline[m][6] = f[m];
...
void ExecutionNVE::array2spline()
{
